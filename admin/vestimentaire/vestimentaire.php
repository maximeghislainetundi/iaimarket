...
$vestimentaire = "vestimentaire";
  ...
  $reqrecep1->execute(array($_SESSION['admin'], $limit, $vestimentaire));


  $debu = $reqrecep1->rowcount();
  if ($debu == 0) {
    header("location:../vestimentaire");
  }
      ...
      <div class="row">
       ...
       <br><br><br>
        <div class="col-lg-8 col-xs-12 col-md-9 col-sm-10 main-section" id="contenu">
          <h2 align="center">Produits vestimentaire</h2>
...
$reqLimit->execute(array($_SESSION['admin'], $lastid, $vestimentaire));
...
$reqLimitpre->execute(array($_SESSION['admin'], $vestimentaire));
...
<center><?php if(($limit < $req[0]) AND ((intval($req[0])-6) != $first)){ ?><a href="../vestimentaire/vestimentaire.php?limit=<?php echo($limitt);?>" class="btn-success btn-lg"><<-- Precedent</a> <?php } if($coun > 0){ ?><a href="../vestimentaire/vestimentaire.php?limit=<?php echo($lastid);?>" class="btn-danger btn-lg">Suivant -->></a><?php }
?></center>
