...
$ali = "aliment";
  ...
  $reqrecep1->execute(array($_SESSION['admin'], $limit, $ali));



  $debu = $reqrecep1->rowcount();
  if ($debu == 0) {
    header("Location:../alimentation");
  }
...
$reqLimit->execute(array($_SESSION['admin'], $lastid, $ali));
...
$reqLimitpre->execute(array($_SESSION['admin'], $ali));
...
<center><?php if(($limit < $req[0]) AND ((intval($req[0])-6) != $first)){ ?><a href="../alimentation/aliment.php?limit=<?php echo($limitt);?>" class="btn-success btn-lg"><<-- Precedent</a> <?php } if($coun > 0){ ?><a href="../alimentation/aliment.php?limit=<?php echo($lastid);?>" class="btn-danger btn-lg">Suivant -->></a><?php }
?></center>
